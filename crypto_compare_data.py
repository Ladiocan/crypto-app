...
# Data limită pentru descărcare (1 ianuarie 2010)
DATA_LIMITA = datetime(2010, 1, 1)
...
def obtine_perechi_binance():
    """
    Obține lista completă a perechilor de pe Binance care includ USDC.
    """
    url = "https://api.binance.com/api/v3/exchangeInfo"
    response = requests.get(url)
    data = response.json()
    perechi = [item["symbol"].replace("USDC", "") for item in data["symbols"]
               if "USDC" in item["symbol"] and item["status"] == "TRADING"]
    return perechi

if __name__ == "__main__":
    perechi = obtine_perechi_binance()
